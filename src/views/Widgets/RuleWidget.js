...
class RuleWidget extends Component {
  render() {
    const { className, cssModule, header, mainText, eventIcon, actionIcon, color, footer, link, children, variant, ...attributes } = this.props;

    // demo purposes only
    const padding = { card: 'p-3', icon: 'p-3' , eventIcon: 'p-3', lead: 'mt-0' };

    const card = { style: 'clearfix', color: color, eventIcon: eventIcon, actionIcon: actionIcon, classes: '' };
    ...
    return (
        <Card>
          <CardBody className={card.classes} {...attributes}>
            {blockIcon(card.eventIcon)}
            {blockIcon(card.actionIcon)}
